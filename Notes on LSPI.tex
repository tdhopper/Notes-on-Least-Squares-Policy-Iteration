...
\todo[inline]{Explain why this is what we want.}
...
% \(
% 	\mathbf{A}^\intercal\mathbf{A}\hat{\mathbf{x}}=\mathbf{A}^\intercal\mathbf{b}.
% \) 
% And if $	(\mathbf{A}^\intercal\mathbf{A})^{-1}$ exists, $\hat{\mathbf{x}}=	(\mathbf{A}^\intercal\mathbf{A})^{-1}\mathbf{A}^\intercal\mathbf{b}$.
